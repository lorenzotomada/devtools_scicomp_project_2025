...
def deflate_eigenpairs(D, v, beta, tol_factor=1e-12):
    ...
    norm_T = np.linalg.norm(np.diag(D)+beta*np.outer(v, v))  # Normalize tolerance to matrix size
...
def parallel_tridiag_eigen(diag, off, comm=None, tol_factor=1e-16, min_size=1,  depth=0, profiler=None, tol_QR=1e-8, max_iterQR=5000):
    ...
    if color == 0:
        eigvals_left, eigvecs_left = parallel_tridiag_eigen(diag1, off1, comm=subcomm, tol_factor=tol_factor, min_size=min_size,  depth=depth+1, profiler=profiler)
    else:
        eigvals_right, eigvecs_right = parallel_tridiag_eigen(diag2, off2, comm=subcomm, tol_factor=tol_factor, min_size=min_size,  depth=depth+1, profiler=profiler)
    ...
    if rank == 0:
        ...
        deflated_eigvals, deflated_eigvecs, D_keep, v_keep,P = deflate_eigenpairs(D, v_vec, beta, tol_factor)
    ...
    (n_yfull_rows, n_yfull_cols,
     n_left_rows, left_cols,
     n_right_rows, right_cols) = comm.bcast(
        (n_yfull_rows, n_yfull_cols,
         n_left_rows, left_cols,
         n_right_rows, right_cols),
        root=0
    )
...
if __name__ == "__main__":
    ...
    t_s=time()
    eigvals, eigvecs = parallel_tridiag_eigen(main_diag, off_diag, comm=comm, min_size=1, tol_factor=1e-10 )
    t_e=time()
